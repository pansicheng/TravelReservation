...
/**
 * exit(0): test passed (1): internal test error (incl. any uncaught exceptions)
 * (2): test failed
 *
 * Test script syntax: #threads line*
 *
 * line: xid launch TM|RMFlights|RMRooms|RMCars|RMCustomers|WC|ALL call func
 * param* return val? except exctype? sleep millisec exit
 */
...
public class Client {
	...
	private static BufferedReader scriptReader = new BufferedReader(new InputStreamReader(System.in));
	...
	private static void launch(String who) {
		...
		String[] rmiNames = new String[] { TransactionManager.RMIName, ResourceManager.RMINameFlights,
				ResourceManager.RMINameRooms, ResourceManager.RMINameCars, ResourceManager.RMINameCustomers,
				WorkflowController.RMIName };
		String[] classNames = new String[] { "TransactionManagerImpl", "ResourceManagerImpl", "ResourceManagerImpl",
				"ResourceManagerImpl", "ResourceManagerImpl", "WorkflowControllerImpl" };
				...
				try {
					Runtime.getRuntime()
							.exec(new String[] { "sh", "-c",
									"java -classpath .. -DrmiPort=" + rmiPort + " -DrmiName=" + rmiNames[i]
				...
				if (action.equals("launch")) {
					...
					for (int i = 0; i < params.length; i++) {
						...
						if (param.startsWith("\"")) {
							...
							params[i] = new Vector();
							StringTokenizer st1 = new StringTokenizer(param.substring(1, param.length() - 1), ",");
					...
					try {
						...
						System.out
								.println("[" + id + "] " + methodName + " exceptioned: " + retExc.getClass().getName());
					}
					...
					if (action.equals("return")) {
						...
						if (st.hasMoreTokens()) {
							...
							if (!retVal.toString().equals(expVal)) {
								System.err.println("[" + id + "] Return value mismatch: expecting " + expVal + "; got "
										+ retVal.toString());
								cleanUpExit(2);
							}
						}
						...
						if (st.hasMoreTokens()) {
							...
							try {
								if (!Class.forName(expExc).isInstance(retExc)) {
									System.err.println(
											"[" + id + "] Exception mismatch: expecting " + expExc + "; got " + retExc);
									cleanUpExit(2);
								}
